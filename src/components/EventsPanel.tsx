        ...
        <div className="space-y-3">
          {gameSession.active_events.map((event: any) => {
            ...
            return (
              <div
                key={event.id}
                className="p-3 rounded-lg border border-destructive/30 bg-card"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <h4 className="font-semibold text-destructive">{event.title}</h4>
                    <p className="text-sm text-muted-foreground mt-1">{event.description}</p>
                    <p className="text-xs text-warning mt-1">{event.effect}</p>
                  </div>
                  <div className="flex items-center gap-1 text-sm font-mono bg-destructive/10 px-2 py-1 rounded">
